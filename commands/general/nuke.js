module.exports = ({
name: "nuke",
code: `$djsEval[module.exports = async ({ Constants: { Colors, Text }, client }, documents, msg, commandData) => {
	if (msg.suffix) {
		let num, query;
		...
		msg.channel.bulkDelete(messages, true).then(({ size }) => {
			msg.send({
				embed: {
					...
					description: `Eliminados ${size} mensajes message${size === 1 ? "" : "s"} en este canal 🗑🔥`,
				},
			});
		}).catch(err => {
			logger.debug(`Error al nukear el canalin channel '${msg.channel.name}' on server '${msg.guild.name}'`, { svrid: msg.guild.id, chid: msg.channel.id, usrid: msg.author.id }, err);
			msg.send({
				embed: {
